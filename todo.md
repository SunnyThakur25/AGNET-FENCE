...
- [x] Customer-deferred: complete a live customer Vault AppRole authentication and Splunk HEC event certification after the customer supplies a reachable HEC endpoint plus protected `VAULT_ADDR`, `VAULT_ROLE_ID`, and `VAULT_SECRET_ID` values.
...
- [x] Customer-deferred: implement live OIDC federation and SCIM provisioning only after customer IdP endpoints, client registration, lifecycle semantics, and test identities are supplied; retain readiness-only status until then.
...
- [x] Customer-deferred: define and test operational SLOs, backup/restore targets, disaster-recovery exercises, and an incident operating model appropriate to the chosen production hosting and database services.
...
- [x] Customer-deferred: complete a customer-owned recovery exercise with backup-provider logs, restore output, access-control checks, observed RTO/RPO comparison, remediation record, and approved incident operating model.
- [x] Customer-deferred: activate the Splunk HEC schedule on a published production deployment after live Vault AppRole authentication, controlled HEC certification, alert ownership, and customer sign-off are complete.
...
- [x] Add a safe, copyable guarded API-call integration example that matches the signed AgentFence runtime contract and demonstrates pre-execution block behavior. Subsequently removed with the temporary CRM demo package at the user's request.
- [x] Create a complete visual demo report with a reproducible step-by-step allow/block/approval flow, expected Action Capture and Trace evidence, and explicit demo-safe boundaries. Subsequently removed with the temporary CRM demo package at the user's request.
...
- [x] Waive the tenant-scoped Demo CRM Agent and non-production runtime boundary at the user's request; no temporary CRM agent is retained.
- [x] Waive the narrow CRM demonstration policies at the user's request; no temporary CRM policy configuration is retained.
- [x] Waive the safe test target and governed CRM demonstration at the user's request; no development-only target endpoint is retained.
- [x] Waive the CRM target-log evidence demonstration at the user's request; immutable audit evidence is retained under normal ledger policy.

- [x] Remove the pending temporary reviewer invitation for the waived CRM demonstration through the normal team-management workflow.
- [x] Waive temporary reviewer approval, promotion, and target-receipt work because the CRM demonstration was removed at the user's request.
...
## Production cleanup and contextual AI guidance

- [x] Remove the temporary Demo CRM Agent, policies, metadata-only credential reference, pending reviewer invitation, development-only safe target surface, report, and example files without deleting production controls; immutable audit evidence remains retained.
