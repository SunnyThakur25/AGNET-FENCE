...
- **Now:** multi-tenant action governance, strongest inbound/outbound Data Guard gating, approvals, action observability, OWASP-controlled assessment, compliance evidence, scoped runtime credential controls, secure connector profiles with controlled Splunk certification, customer-controlled Vault AppRole activation, and public-HTTPS MCP discovery/trust/proxy controls. **Next:** continuous approved connector delivery, deeper target connectors, richer policy packs, private-network and STDIO MCP transport, OAuth authorization-code integration, and security-workflow integrations.
