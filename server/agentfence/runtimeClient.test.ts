...
const state = vi.hoisted(() => ({
  decision: { allowed: false, reason: "Blocked by demo policy", toolCallId: 81, redactedOutboundPayload: { customerIds: ["demo-customer-001"] } },
...
const request = {
  ...
  outboundPayload: { customerIds: ["demo-customer-001"] },
...
describe("AgentFence runtime client guarded delivery", () => {
  it("does not call the external delivery callback when AgentFence returns a block", async () => {
    state.decision = { allowed: false, reason: "Blocked by demo policy", toolCallId: 81, redactedOutboundPayload: { customerIds: ["demo-customer-001"] } };
    state.outcomes = [];
    ...
    await expect(runtime.guardAndDeliver(request, deliver)).rejects.toThrow("AgentFence blocked crm.customer.export: Blocked by demo policy");
    expect(deliver).not.toHaveBeenCalled();
    expect(state.outcomes).toEqual([]);
  });

  it("calls the external delivery callback only after an allow and reports the target outcome", async () => {
    state.decision = { allowed: true, reason: "Allowed by narrow demo policy", toolCallId: 82, redactedOutboundPayload: { customerIds: ["demo-customer-001"] } };
    state.outcomes = [];
    const deliver = vi.fn(async (payload: unknown) => ({ accepted: true, payload }));
    const runtime = createAgentFenceRuntimeClient({ endpoint: "https://agentfence.example.test/", credential: "x".repeat(40) });
    await expect(runtime.guardAndDeliver(request, deliver)).resolves.toEqual({ accepted: true, payload: { customerIds: ["demo-customer-001"] } });
