...
The current Native MCP Gateway supports public HTTPS MCP server registration, JSON-RPC tool discovery, administrator trust, per-tool enablement, signed runtime scope, Data Guard and policy enforcement, and proxied `tools/call` execution. It does not claim STDIO transport, private-network connectivity, OAuth authorization-code handoff, dynamic client registration, or upstream streaming support; those remain roadmap work.
